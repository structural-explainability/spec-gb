...
from spec_gb.paths import data_spec_path, repo_root
...
def test_scope_exclusions_export_is_non_empty() -> None:
    ...
    root = repo_root()
    export = build_scope_exclusions_export(version=__version__, root=root)
...
def test_conformance_export_contains_canonical_identifiers() -> None:
    ...
    root = repo_root()
    export = build_conformance_export(version=__version__, root=root)
...
def test_generated_exports_are_current() -> None:
    ...
    root = repo_root()
    export_all(version=__version__, root=root, check=True)
...
def test_generated_requirements_json_matches_builder() -> None:
    ...
    expected = asdict(build_requirements_export(version=__version__, root=root))
...
def test_generated_scope_exclusions_json_matches_builder() -> None:
    ...
    expected = asdict(build_scope_exclusions_export(version=__version__, root=root))
    actual = json.loads(path.read_text(encoding="utf-8"))
...
def test_generated_conformance_checks_json_matches_builder() -> None:
    ...
    expected = asdict(build_conformance_export(version=__version__, root=root))
    actual = json.loads(path.read_text(encoding="utf-8"))
